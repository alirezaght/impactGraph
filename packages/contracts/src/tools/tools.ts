...
export const MCP_TOOL_CONTRACTS = {
  ...
  analyze_impact: {
    description:
      'Build an evidence-backed impact analysis for a submitted specification and return a BOUNDED summary: status, extraction quality, index freshness, coverage, counts, the top structural impacts, unmatched requirements, unresolved concepts, blocking questions and important warnings. Validates repository coverage first and auto-indexes registered repositories missing from the current index. When coverage is fundamentally insufficient, workspaceCoverage.status is "insufficient-coverage", the readiness score is WITHHELD, and requiredActions states machine-readable next steps (index/register repositories, confirm candidates, refresh the index, or report limited scope) — follow them instead of treating the partial result as the answer. Lexical-only matches are excluded by default. topImpacts contains PLANNING IMPACTS only — surfaces with evidence they matter to satisfying the specification; components that are merely reachable from the change are summarised in dependencyContext and paged from list_impacts with roles, never mixed into the primary answer. unresolvedSurfaces reports specification terms that resolve to no indexed artifact, each with the reading the evidence supports and the readings that stay open — an absent surface is a finding, not a gap. Use list_impacts for the full paginated detail. Persists a new draft analysis.',
    input: impactFiltersSchema.extend({ specificationId: z.string().min(1) }).strict(),
    output: cliImpactSummarySchema,
  },
  list_impacts: {
    description:
      "Page through the impacts of a stored analysis with dependency paths, evidence bases and §14 confidence signals. Supports topN, minLikelihood, evidenceTypes, includeLexicalOnly, includeExcluded, requirementId, roles and a pagination cursor. `roles` defaults to ['planning-impact'] — the planning decisions. Pass roles: ['dependency-context'] for components that are structurally reachable from the change with no evidence they must change, ['investigation-lead'] for name and meaning matches, or all three for the complete audit set. The applied filters are always echoed back in pagination.appliedFilters.",

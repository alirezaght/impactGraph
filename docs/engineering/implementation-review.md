...
## The result categories (PRD §24.1, ADR-0022, ADR-0023)
...
when the specification states a design choice; `preserve` when a preservation requirement
(`Requirement.intent === 'preserve'`, ADR-0023) drew a regression boundary around it).
...
| **Guard violated**     | A preservation requirement protected this surface — the specification said it must remain unchanged — and the diff modified it. Distinct from Divergent on purpose: divergence is "changed differently than planned", a revisable design judgement, whereas a regression boundary is the AUTHOR forbidding the change. Ranked second in the discrepancy order, behind Missing and ahead of Divergent. The mirror case — a protected surface the diff left alone — is Reuse confirmed, worded "regression boundary held" (never "planned reuse") and counted as satisfied coverage: that verification is the whole point of stating a boundary |
...
**A guard is not a non-goal.** A non-goal EXCLUDES a component from the analysis (`excluded`
likelihood; a change there is Unexpected, with the contradicted non-goal named). A preservation
requirement keeps the component IN scope, expects it to be exercised, and requires no behavioural
diff. Both appear in the same documents — "No backfill of previously missed editions" beside
"Existing send behaviour must remain unchanged" — and collapsing either into the other inverts what
the report says. Section role decides which is which: preservation-worded text under a Non-goals
heading stays a non-goal.
